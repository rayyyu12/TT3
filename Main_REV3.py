...
MASTER_ENGINE_VOL = 0.99 # Overall engine sound volume (production level)
...
HELLCAT_REDLINE_RPM = 4500  # Reduced from 6200 for easier testing of upshift/downshift logic
...
class HellcatEngineSimulation:
    ...
    def update(self, dt, new_raw_throttle):
        ...
        if self.state == "ENGINE_OFF":
            ...
            # Check if startup is almost finished for smooth crossfade
            startup_sound = self.sm.sounds.get('startup')
            if startup_sound and self.sm.channel_startup.get_busy():
                # Get the position/time remaining in startup sound
                startup_time_elapsed = current_time - self.state_start_time
                estimated_startup_duration = startup_sound.get_length()
                
                # Start crossfade in the last 0.5 seconds of startup
                if startup_time_elapsed >= estimated_startup_duration - 0.5:
                    # Begin idle sound at low volume for crossfade
                    self.sm.play_foundation_layer(0.0, HELLCAT_IDLE_RPM) 
                    
            if not self.sm.is_startup_busy():
                print("\nHellcat Engine Idling.")
                self.state = "IDLE"
                self.state_start_time = current_time
                # Ensure idle sound is playing at proper volume
                self.sm.play_foundation_layer(0.0, HELLCAT_IDLE_RPM)
